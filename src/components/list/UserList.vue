...
const onClickSendVerificationEmail = (email: string) => {
  return userApi.POST_sendVerificationEmail(email).then(() =>
    alert('Email Sent!')
  ).catch((err) =>
    alert(err.message)
  )
...
<template>
  <section v-if="tableType === E_UserListType.checkbox">
    <v-data-table v-model="model" :items-per-page="0" :headers="checkboxHeader" :items="props.userList"
      :sort-by="[{ key: 'createdAt', order: 'asc' }]" show-select return-object>
      <template #bottom></template>
    </v-data-table>
  </section>

  <section v-else>
    <v-data-table :items-per-page="0" :headers="headers" :items="props.userList.map((user) => ({
    ...user,
    createdAt: user.createdAt && formatDateTime(user.createdAt),
  }))
    " :sort-by="[{ key: 'createdAt', order: 'asc' }]">
      <template v-slot:item.actions="{ item }">
        <v-select v-model="item.role" :items="Object.values(E_Role)"
          @update:modelValue="emit('onChangeSelectRole', item.id, item.role)" hide-details>
        </v-select>
        <v-select v-model="item.userStatus" :items="Object.values(E_UserStatus)" @update:modelValue="
    emit('onChangeUserStatus', item.id, item.userStatus)
    " hide-details></v-select>
      </template>
      <template v-slot:item.verification="{ item }">
        <v-btn v-if="item.userStatus !== E_UserStatus.ok" @click=" onClickSendVerificationEmail(item.email)">인증 메일
          보내기
        </v-btn>
        <p v-else>
          인증 완료
